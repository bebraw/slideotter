...
2. Continue ADR 0005 beyond the now-implemented divider, quote, photo, deck-local/favorite layout library, library-backed layout candidates, and `mediaItems` data model: add photo-grid support and generated layout-definition workflows so common requests such as split-photo slides map to validated structured specs and shareable JSON layout definitions.
