...
2. Continue ADR 0005 beyond the now-implemented divider, quote, photo, manual photo-grid creation, deck-local/favorite layout library, layout JSON exchange, library-backed layout candidates, direct candidate-to-layout saving, `mediaItems` data model, and explicit family-changing structure candidates: add generated layout-definition workflows so common requests such as split-photo slides map to validated structured specs and shareable JSON layout definitions.
