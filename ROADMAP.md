...
2. Use the real paper-to-talk workflow from proposed ADR 0064 and ADR 0065 as the next concrete source-grounding test. First pin and validate the private `@kirjolab/paper-import@0.1.0` artifact through a thin Slideotter adapter and its conformance-v2 corpus; then review a LaTeX/PDF source pair, approve a compact digest and timed outline, and draft from locator-rich evidence and original figure candidates. Keep the first slice local-first and defer OCR, embeddings, a general research library, and mandatory visible citations.
