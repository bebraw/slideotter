...
- [`docs/adr/proposed/0064-reviewed-latex-and-pdf-paper-import.md`](./docs/adr/proposed/0064-reviewed-latex-and-pdf-paper-import.md) for reviewed, fingerprint-bound LaTeX/PDF paper intake through the private versioned `@kirjolab/paper-import` adapter boundary without importing its application runtime
- [`docs/adr/proposed/0065-timed-paper-to-presentation-creation.md`](./docs/adr/proposed/0065-timed-paper-to-presentation-creation.md) for duration-first paper digests, evidence-aware outlines, and progressive paper-to-talk drafting through the existing staged workflow
- [`docs/adr/implemented/0066-local-codex-gateway.md`](./docs/adr/implemented/0066-local-codex-gateway.md) for the manual loopback gateway that lets local Studio generation use Codex-owned ChatGPT or API-key authentication without replacing the direct OpenAI provider
...
- [`docs/adr/implemented/0052-agent-action-timeline.md`](./docs/adr/implemented/0052-agent-action-timeline.md) for showing live agent workflow steps during real candidate, validation, and rebuild flows
...
- [`docs/adr/implemented/0060-licensed-narration-avatar-overlay.md`](./docs/adr/implemented/0060-licensed-narration-avatar-overlay.md) for the implemented optional copyright-safe comic presenter overlay that reacts to reviewed narration without adding generated video to core
...
- Local Codex access runs through ADR 0066's separately started, bearer-token-protected loopback gateway and the server-owned OpenAI-compatible provider path. Its required `CODEX_GATEWAY_CODEX_HOME` becomes the child's synthetic home and uses a separately authenticated file-backed credential, excluding personal and repository Codex customization. Codex-owned system skills, vendor-bundled skills, and machine-admin `/etc/codex` inputs remain trusted. Codex owns upstream authentication; the browser never receives ChatGPT or OpenAI credentials, and hosted Workers cannot use this local process.
...
2. Use the real paper-to-talk workflow from proposed ADR 0064 and ADR 0065 as the next concrete source-grounding test. The immutable private `@kirjolab/paper-import@0.1.3` artifact, state-free adapter, fixed PDF.js boundary, provenance quarantine, and conformance-v2 proof are now in place. Next normalize locator-rich paper types and build reviewed draft upload/preview before connecting confirmation, a compact digest, timed outline, or progressive drafting. Keep the workflow local-first and defer OCR, embeddings, a general research library, and mandatory visible citations.
...
7. Extend SVGL logo search from ADR 0054 only when real decks need refinements beyond the implemented bounded workflow: explicit user selection, outline-stage suggestions, presentation-local imports, provenance metadata, SVG sanitization, and normal media validation before slide use.
...
10. Harden the Electron wrapper from ADR 0033 from real macOS desktop usage, especially icons, signing/notarization, release documentation, and packaged export validation. After ADR 0066's manual Codex gateway path proves reliable, consider Electron-owned gateway lifecycle, ephemeral port/token wiring, and Codex auth-status UX without exposing credentials to the renderer.
...
19. Extend hypermedia knowledge memory from ADR 0061 and ADR 0062 only where real decks need it beyond the implemented presentation-scoped baseline: restore/merge/link controls in the Memory workbench, derived outline proposals from selected memory, dependency-aware edit review, explicit knowledge-pack import/export, and cross-presentation reuse that preserves preview, compare, apply, and validate boundaries.
