...
6. complete media validation mode now adds rendered-media checks for small, clipped, upscaled, distorted, unlabeled, unloaded, dimensionless, text-overlapping, or progress-area-crowding visuals plus caption/source attachment, minimum spacing, and maximum attachment distance
...
1. keep hardening complete media-validation mode beyond its current media legibility, slide bounds, progress-area spacing, and caption/source spacing checks, now covered by a fixture in the quality gate, especially once media-heavy slide families land
2. keep extending shared deck-context patches if new deck-plan modes are added; the current sequence, boundary, decision, operator, compressed, composed, and deck-authoring candidates all carry shared-context steering, and generated content scaffolds use prose evidence items instead of metric-style placeholders, enforced by the deck-plan fixture in the quality gate
