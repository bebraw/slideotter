...
1. keep hardening multiple-presentation workflows now that create, select, duplicate, delete, active export naming, active archive publishing, per-presentation render baselines, per-presentation preview artifacts, browser workflow smoke coverage, and compact selector metadata exist; the next useful slice is improving multi-deck discoverability once real usage reveals whether search, sorting, or tagging is needed
