...
const exactAuthoringMetaLabels = new Set([
  "accessibility check",
  "accessible language",
...
const authoringMetaPatterns = [
  ...
  /\bensure\b.*\bhighlighted\b.*\bwithout\b/,
  ...
  /\bdo not use\b.*\boverly technical\b/,
  ...
  /\bkeep descriptions\b/,
  /\bkeep descriptions\b.*\bconcrete\b.*\bspecific\b/,
  /\bkeep descriptions\b.*\bfocused\b/,
...
export function isAuthoringMetaText(value: unknown): boolean {
  ...
  if (exactAuthoringMetaLabels.has(text)) {
    return true;
  }

  return authoringMetaPatterns.some((pattern) => pattern.test(text));
}
