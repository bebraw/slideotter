...
type LocalDeckStructureInput = {
  fallbackTitles: string[];
  structureContext: DeckStructureContext;
};

function createDecisionNarrativeSlideSpec(
  currentContext: DeckStructureContext,
  details: DeckWideAuthoringDetails
): JsonObject {
  ...
  switch (baseSpec.type) {
    ...
    case "quote":
      return createDecisionNarrativeQuoteSpec(currentContext, details);
    case "photo":
      return createDecisionNarrativePhotoSpec(details);
    case "photoGrid":
      return createDecisionNarrativePhotoGridSpec(details);
    case "cover":
      return createDecisionNarrativeCoverSpec(currentContext, details);
    case "toc":
      return createDecisionNarrativeTocSpec(currentContext, details);
    case "content":
      return createDecisionNarrativeContentSpec(currentContext, details);
    case "summary":
      return createDecisionNarrativeSummarySpec(currentContext, details);
    default:
      return baseSpec;
  }
}

function createDecisionNarrativeQuoteSpec(currentContext: DeckStructureContext, details: DeckWideAuthoringDetails): JsonObject {
  return rewriteQuoteSlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
    attribution: "Decision path",
    context: `Use this pull quote to keep ${currentContext.audience} focused on the approval call.`,
    quote: "A deck earns trust when the decision, proof, and next move stay visible.",
    source: "Authored deck copy"
  });
}

function createDecisionNarrativePhotoSpec(details: DeckWideAuthoringDetails): JsonObject {
  return rewritePhotoSlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
    caption: "Use the image as visual evidence for the decision, proof, and next move."
  });
}

function createDecisionNarrativePhotoGridSpec(details: DeckWideAuthoringDetails): JsonObject {
  return rewritePhotoGridSlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
...
function createDecisionNarrativeCoverSpec(currentContext: DeckStructureContext, details: DeckWideAuthoringDetails): JsonObject {
  return rewriteCoverSlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
    cards: [
      {
        body: `State the decision clearly for ${currentContext.audience} before showing tooling detail.`,
        title: "Decision"
      },
      {
    ...
    summary: `Frame the presentation as one decision-support path for ${currentContext.audience}.`
  });
}

function createDecisionNarrativeTocSpec(currentContext: DeckStructureContext, details: DeckWideAuthoringDetails): JsonObject {
  return rewriteTocSlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
    cards: [
      {
    ...
    summary: `Map the deck as a short path from framing through proof to the next move for ${currentContext.audience}.`
  });
}

function createDecisionNarrativeContentSpec(currentContext: DeckStructureContext, details: DeckWideAuthoringDetails): JsonObject {
  return rewriteContentSlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
    eyebrow: "Evidence",
    guardrails: [
      {
...
function createDecisionNarrativeSummarySpec(currentContext: DeckStructureContext, details: DeckWideAuthoringDetails): JsonObject {
  return rewriteSummarySlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
    bullets: [
      {
    ...
    summary: `Close on an explicit approval path so ${currentContext.audience} can act on the deck without guessing.`
  });
}

function createOperatorHandoffSlideSpec(
  currentContext: DeckStructureContext,
  details: DeckWideAuthoringDetails
): JsonObject {
  const baseSpec = details.currentSpec;
  ...
  switch (baseSpec.type) {
    case "divider":
      return rewriteDividerSlideSpec(baseSpec, details.proposedIndex, details.proposedTitle);
    case "quote":
      return createOperatorHandoffQuoteSpec(currentContext, details);
    case "photo":
      return createOperatorHandoffPhotoSpec(details);
    case "photoGrid":
      return createOperatorHandoffPhotoGridSpec(details);
    case "cover":
      return createOperatorHandoffCoverSpec(currentContext, details);
    case "toc":
      return createOperatorHandoffTocSpec(details);
    case "content":
      return createOperatorHandoffContentSpec(currentContext, details);
    case "summary":
      return createOperatorHandoffSummarySpec(details);
    default:
      return baseSpec;
  }
}

function createOperatorHandoffQuoteSpec(currentContext: DeckStructureContext, details: DeckWideAuthoringDetails): JsonObject {
  return rewriteQuoteSlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
    attribution: "Operator handoff",
    context: `Use this pull quote to keep ${currentContext.objective} attached to validation and ownership.`,
    quote: "A maintained deck keeps the source, preview, and validation path in the same loop.",
    source: "Authored deck copy"
  });
}

function createOperatorHandoffPhotoSpec(details: DeckWideAuthoringDetails): JsonObject {
  return rewritePhotoSlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
    caption: "Keep the image attached to the source, preview, and validation loop."
  });
}

function createOperatorHandoffPhotoGridSpec(details: DeckWideAuthoringDetails): JsonObject {
  return rewritePhotoGridSlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
...
function createOperatorHandoffCoverSpec(currentContext: DeckStructureContext, details: DeckWideAuthoringDetails): JsonObject {
  return rewriteCoverSlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
    cards: [
      {
    ...
    summary: `Frame the deck as an operator handoff for ${currentContext.audience}.`
  });
}

function createOperatorHandoffTocSpec(details: DeckWideAuthoringDetails): JsonObject {
  return rewriteTocSlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
    cards: [
      {
...
function createOperatorHandoffContentSpec(currentContext: DeckStructureContext, details: DeckWideAuthoringDetails): JsonObject {
  return rewriteContentSlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
...
function createOperatorHandoffSummarySpec(details: DeckWideAuthoringDetails): JsonObject {
  return rewriteSummarySlideSpec(details.currentSpec, details.proposedIndex, details.proposedTitle, {
    bullets: [
      {
...
function createSequenceLedStructureCandidate(input: LocalDeckStructureInput): JsonObject {
  const { fallbackTitles, structureContext } = input;

  return createDeckStructurePlan(structureContext, {
...
function createBoundaryLedStructureCandidate(structureContext: DeckStructureContext): JsonObject {
  return createDeckStructurePlan(structureContext, {
...
function createDecisionLedStructureCandidate(structureContext: DeckStructureContext): JsonObject {
  return createDeckStructurePlan(structureContext, {
...
function createOperatorChecklistStructureCandidate(input: LocalDeckStructureInput): JsonObject {
  const { fallbackTitles, structureContext } = input;
...
function createCompressedProofStructureCandidate(input: LocalDeckStructureInput): JsonObject {
  const { fallbackTitles, structureContext } = input;
...
function createComposedDecisionHandoffCandidate(structureContext: DeckStructureContext): JsonObject {
  return createDeckStructurePlan(structureContext, {
...
function createDecisionNarrativeAuthoringCandidate(structureContext: DeckStructureContext): JsonObject {
  return createDeckWideAuthoringPlan(structureContext, {
    ...
    createSlideSpec: createDecisionNarrativeSlideSpec,
    focus: [
...
function createOperatorHandoffAuthoringCandidate(structureContext: DeckStructureContext): JsonObject {
  return createDeckWideAuthoringPlan(structureContext, {
    ...
    createSlideSpec: createOperatorHandoffSlideSpec,
    focus: [
...
export function createLocalDeckStructureCandidates(context: DeckContext): JsonObject[] {
  ...
  const input = { fallbackTitles, structureContext };

  return [
    createSequenceLedStructureCandidate(input),
    createBoundaryLedStructureCandidate(structureContext),
    createDecisionLedStructureCandidate(structureContext),
    createOperatorChecklistStructureCandidate(input),
    createCompressedProofStructureCandidate(input),
    createComposedDecisionHandoffCandidate(structureContext),
    createDecisionNarrativeAuthoringCandidate(structureContext),
    createOperatorHandoffAuthoringCandidate(structureContext)
  ];
}
