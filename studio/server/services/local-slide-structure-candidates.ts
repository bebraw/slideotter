...
function createCardSequenceStructureVariant(currentSpec: SlideSpec, structureContext: StructureContext, cards: JsonObject[]): LocalStructureVariant {
  return {
...
function createCardBoundaryStructureVariant(currentSpec: SlideSpec, structureContext: StructureContext, cards: JsonObject[]): LocalStructureVariant {
  return {
...
function createCardHandoffStructureVariant(currentSpec: SlideSpec, structureContext: StructureContext, cards: JsonObject[]): LocalStructureVariant {
  return {
...
function createCardStructureCandidates(currentSpec: SlideSpec, structureContext: StructureContext, options: OperationOptions = {}): Candidate[] {
  ...
  return mapLocalStructureVariants([
    createCardSequenceStructureVariant(currentSpec, structureContext, cards),
    createCardBoundaryStructureVariant(currentSpec, structureContext, cards),
    createCardHandoffStructureVariant(currentSpec, structureContext, cards)
  ], [
    `${currentSpec.type} slide`,
...
function createContentSequenceStructureVariant(currentSpec: SlideSpec, structureContext: StructureContext, guardrails: JsonObject[], signals: JsonObject[]): LocalStructureVariant {
  return {
    label: "Sequence structure",
...
function createContentBoundaryStructureVariant(currentSpec: SlideSpec, guardrails: JsonObject[], signals: JsonObject[]): LocalStructureVariant {
  return {
    label: "Boundary structure",
...
function createContentDecisionStructureVariant(currentSpec: SlideSpec, structureContext: StructureContext, guardrails: JsonObject[], signals: JsonObject[]): LocalStructureVariant {
  return {
...
function createContentStructureCandidates(currentSpec: SlideSpec, structureContext: StructureContext, options: OperationOptions = {}): Candidate[] {
  ...
  return mapLocalStructureVariants([
    createContentSequenceStructureVariant(currentSpec, structureContext, guardrails, signals),
    createContentBoundaryStructureVariant(currentSpec, guardrails, signals),
    createContentDecisionStructureVariant(currentSpec, structureContext, guardrails, signals)
  ], [
    `${currentSpec.type} slide`,
...
function createSummaryOperatingStructureVariant(currentSpec: SlideSpec, structureContext: StructureContext, bullets: JsonObject[], resources: JsonObject[]): LocalStructureVariant {
  return {
...
function createSummaryOwnershipStructureVariant(currentSpec: SlideSpec, structureContext: StructureContext, bullets: JsonObject[], resources: JsonObject[]): LocalStructureVariant {
  return {
...
function createSummaryHandoffStructureVariant(currentSpec: SlideSpec, structureContext: StructureContext, bullets: JsonObject[], resources: JsonObject[]): LocalStructureVariant {
  return {
    label: "Handoff structure",
...
function createSummaryStructureCandidates(currentSpec: SlideSpec, structureContext: StructureContext, options: OperationOptions = {}): Candidate[] {
  ...
  return mapLocalStructureVariants([
    createSummaryOperatingStructureVariant(currentSpec, structureContext, bullets, resources),
    createSummaryOwnershipStructureVariant(currentSpec, structureContext, bullets, resources),
    createSummaryHandoffStructureVariant(currentSpec, structureContext, bullets, resources)
  ], [
    `${currentSpec.type} slide`,
...
function createDividerFamilyChangeCandidate(currentSpec: SlideSpec, structureContext: StructureContext, baseTitle: string, options: OperationOptions): Candidate {
  return createFamilyChangeCandidate(
    currentSpec,
    structureContext,
    {
...
function createQuoteFamilyChangeCandidate(currentSpec: SlideSpec, structureContext: StructureContext, baseTitle: string, textClaim: string, options: OperationOptions): Candidate {
  return createFamilyChangeCandidate(
    currentSpec,
    structureContext,
    {
...
function createPhotoFamilyChangeCandidate(currentSpec: SlideSpec, structureContext: StructureContext, baseTitle: string, mediaItems: JsonObject[], options: OperationOptions): Candidate {
...
function createPhotoGridFamilyChangeCandidate(currentSpec: SlideSpec, structureContext: StructureContext, baseTitle: string, mediaItems: JsonObject[], options: OperationOptions): Candidate {
  return createFamilyChangeCandidate(
    currentSpec,
    structureContext,
    {
...
export function createLocalFamilyChangeCandidates(currentSpec: SlideSpec, structureContext: StructureContext, options: OperationOptions = {}): Candidate[] {
  ...
  if (!["divider"].includes(String(currentSpec.type))) {
    candidates.push(createDividerFamilyChangeCandidate(currentSpec, structureContext, baseTitle, options));
  }

  if (!["quote"].includes(String(currentSpec.type))) {
    candidates.push(createQuoteFamilyChangeCandidate(currentSpec, structureContext, baseTitle, textClaim, options));
  }

  if (currentSpec.type !== "photo" && (currentSpec.media || mediaItems.length)) {
    candidates.push(createPhotoFamilyChangeCandidate(currentSpec, structureContext, baseTitle, mediaItems, options));
  }

  if (currentSpec.type !== "photoGrid" && mediaItems.length >= 2) {
    candidates.push(createPhotoGridFamilyChangeCandidate(currentSpec, structureContext, baseTitle, mediaItems, options));
  }
