...
ADR 0019 is now in progress. The first hosted target lives under `/cloud/` and uses a Cloudflare Worker with Workers Static Assets rather than a separate Pages deployment. The Worker serves the built Vite client from `/studio/client-dist`, routes API requests through the Worker first, exposes `/api/cloud/health`, advertises `/api/cloud/v1`, can read workspace, presentation, slide, and job collections from configured D1/R2 bindings, and supports bearer-token-guarded bootstrap writes for workspace, presentation, slide, and job creation.
