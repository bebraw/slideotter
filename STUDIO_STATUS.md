...
Current implemented decisions cover workflow controls, pragmatic UI, materials, staged creation, browser presentation mode, DOM-first rendering, reversible deck length scaling, source-grounded generation, and the rich slide-family/layout-library baseline. Current proposed decisions cover richer future layout-family direction, user-data packaging, two-dimensional and graph-style presentations, LLM replacement of remaining deterministic workflows, LM Studio model selection, progressive generation preview, hypermedia APIs, and statecharts for graph presentations. See [`docs/adr/README.md`](./docs/adr/README.md) for the full index.
