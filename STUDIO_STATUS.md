...
- After staged creation materializes slides, Studio moves the author directly into Slide Studio. The persistent Theme rail tab opens the optional Theme drawer, where saved favorites, explicit theme candidates, palette/font controls, a three-slide real-deck preview, and an apply summary update deck-level theme state without rewriting slide content. During slide drafting, the server publishes per-slide progress, writes the accumulated deck after each successful slide, and seeds per-slide context from the approved outline and generated slide plan so partial output remains available if a later LLM call fails.
