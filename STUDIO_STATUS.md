...
- Slide Studio now uses a single current-slide workbench with inline variant generation controls. The left rail shows normal slide thumbnails until candidates exist, then switches to a variant candidate rail where the original slide appears first and selected candidates replace the main preview before apply.
...
- Generation diagnostics are collapsed by default and group prompt budgets, source retrieval, and recent workflow events behind one inspectable panel.
