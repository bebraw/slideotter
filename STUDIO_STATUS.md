...
Current implemented decisions cover workflow controls, pragmatic UI, materials, staged creation, browser presentation mode, DOM-first rendering, reversible deck length scaling, source-grounded generation, the rich slide-family/layout-library baseline, and token-efficient project coding. Current proposed decisions cover richer future layout-family direction, user-data packaging, two-dimensional and graph-style presentations, LLM replacement of remaining deterministic workflows, LM Studio model selection, progressive generation preview, hypermedia APIs, statecharts for graph presentations, Cloudflare cloud hosting, a plugin system for optional extensions, PPTX handoff output, selection-scoped chat commands, post-creation theme control, inline current-slide variant generation, assisted check remediation, custom layout authoring with preview, custom HTML/SVG visual support, token-efficient LLM generation, and Cloudflare collaboration. See [`docs/adr/README.md`](./docs/adr/README.md) for the full index.
