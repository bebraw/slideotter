...
- New presentation setup captures a target slide count and initial generation mode alongside audience, tone, objective, constraints, and theme brief; the server generates the first structured deck through the configured LLM when available or local rules when requested/unconfigured, and the saved target feeds the Deck Planning length scaler.
