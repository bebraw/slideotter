...
- New presentation setup captures a target slide count, initial generation mode, font, core palette, audience, tone, objective, constraints, and theme brief; the server generates the first structured deck through the configured LLM when available or local rules when requested/unconfigured, can reuse retrieved source snippets from the active presentation, and the saved target feeds the Deck Planning length scaler.
